...
using Assets.AstarPathfindingProject.Generators.Utilities.Voxels;
using Assets.AstarPathfindingProject.Navmesh;
using Pathfinding.ClipperLib;
using Pathfinding.Poly2Tri;
using UnityEngine;
using IntRect = Assets.AstarPathfindingProject.Core.IntRect;
using Polygon = Pathfinding.Poly2Tri.Polygon;

namespace Assets.AstarPathfindingProject.Generators.Utilities {
	public class TileHandler {
		...
		void CutPoly (Int3[] verts, int[] tris, ref Int3[] outVertsArr, ref int[] outTrisArr, out int outVCount, out int outTCount, Int3[] extraShape, Int3 cuttingOffset, Bounds realBounds, CutMode mode = CutMode.CutAll | CutMode.CutDual, int perturbate = 0) {
			...
			var polyCache = new Stack<Polygon>();
			...
			while ( true ) {
				...
				if ( addIndex == -1 ) {
					...
					int ct;
					ct = Utility.ClipPolygon (clipIn , 3 , clipOut ,  1 , 0, 0);
					if ( ct == 0 ) continue;

					ct = Utility.ClipPolygon (clipOut, ct , clipIn , -1 , 2*clipExtents.x, 0);
					if ( ct == 0 ) continue;

					ct = Utility.ClipPolygon (clipIn, ct , clipOut ,  1 , 0, 2);
					if ( ct == 0 ) continue;

					ct = Utility.ClipPolygon (clipOut, ct , clipIn , -1 , 2*clipExtents.z, 2);
					if ( ct == 0 ) continue;
		...
		void DelaunayRefinement (Int3[] verts, int[] tris, ref int vCount, ref int tCount, bool delaunay, bool colinear, Int3 worldOffset) {
			...
			for (int i=0;i<tCount;i+=3) {
				if (!Core.Polygon.IsClockwise(verts[tris[i]],verts[tris[i+1]],verts[tris[i+2]])) {
					int tmp = tris[i];
					tris[i] = tris[i+2];
					tris[i+2] = tmp;
				}
			...
			for (int i=0;i<tCount;i+=3) {
				for (int j=0;j<3;j++) {
					...
					if (lookup.TryGetValue(new Int2(tris[i+((j+1)%3)], tris[i+((j+0)%3)]), out opp)) {
						...
						if (!Core.Polygon.Left (po,pl,popp) || Core.Polygon.LeftNotColinear(po,pr,popp)) {
