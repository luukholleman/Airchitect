	...
	*/
	...
	public class NavmeshCut : MonoBehaviour {
		...
		public void GetContour (List<List<Pathfinding.ClipperLib.IntPoint>> buffer) {
			...
			switch (type) {
			case MeshType.Rectangle:
				List<Pathfinding.ClipperLib.IntPoint> buffer0 = ListPool<Pathfinding.ClipperLib.IntPoint>.Claim();
				if (useRotation) {
					Matrix4x4 m = tr.localToWorldMatrix;
				...
				break;
			case MeshType.Circle:
				buffer0 = ListPool<Pathfinding.ClipperLib.IntPoint>.Claim(circleResolution);
				if (useRotation) {
					Matrix4x4 m = tr.localToWorldMatrix;
			...
			case MeshType.CustomMesh:
				...
				if (contours != null) {
					...
					for (int i=0;i<contours.Length;i++) {
						...
						buffer0 = ListPool<Pathfinding.ClipperLib.IntPoint>.Claim(contour.Length);
						if (useRotation) {
							Matrix4x4 m = tr.localToWorldMatrix;
		...
		public void OnDrawGizmos () {
			...
			var buffer = ListPool<List<Pathfinding.ClipperLib.IntPoint>>.Claim();
			GetContour(buffer);
